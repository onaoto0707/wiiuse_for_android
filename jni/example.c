...
double forktime_b,forktime_a;
double forktime_buf[1024],elapsedtime_buf[1024];
int count = 0;
/**
...
static int make_input(int keycode){
	int pid;
	int status;
	forktime_b = gettimeofday_sec();
	if( (pid = fork() ) == -1){
		return -1;
	}
	else if(pid == 0){
		...
		case WIIMOTE_BUTTON_A:
			execlp("input", "", "keyevent", KEYCODE_ENTER, NULL);
			break;
		case WIIMOTE_BUTTON_UP:
			execlp("input", "", "keyevent", KEYCODE_DPAD_UP, NULL);
			break;
		case WIIMOTE_BUTTON_DOWN:
			execlp("input", "", "keyevent", KEYCODE_DPAD_DOWN, NULL);
			break;
		case WIIMOTE_BUTTON_LEFT:
			execlp("input", "", "keyevent", KEYCODE_DPAD_LEFT, NULL);
			break;
		case WIIMOTE_BUTTON_RIGHT:
			execlp("input", "", "keyevent", KEYCODE_DPAD_RIGHT, NULL);
			break;
		case WIIMOTE_BUTTON_HOME:
			execlp("input", "", "keyevent", KEYCODE_HOME, NULL);
			break;
		case WIIMOTE_BUTTON_PLUS:
			execlp("input", "", "keyevent", KEYCODE_MENU, NULL);
			break;
		case WIIMOTE_BUTTON_MINUS:
			execlp("input", "", "keyevent", KEYCODE_BACK, NULL);
			break;
		case WIIMOTE_BUTTON_ONE:
			execlp("input", "", "keyevent", KEYCODE_SEARCH, NULL);
			break;
		case WIIMOTE_BUTTON_TWO:
			execlp("input", "", "keyevent", KEYCODE_ESCAPE, NULL);
			break;
		default:
			break;
		}
...
//	printf("\n\n--- EVENT [id %i] ---\n", wm->unid);
	...
	if (IS_PRESSED(wm, WIIMOTE_BUTTON_A)){
		make_input(WIIMOTE_BUTTON_A);
	//	printf("A pressed\n");
	}
	/*
	if (IS_PRESSED(wm, WIIMOTE_BUTTON_B)){
		//make_input(WIIMOTE_BUTTON_B)
		printf("B pressed\n");
	}*/
	if (IS_PRESSED(wm, WIIMOTE_BUTTON_UP))	{
		make_input(WIIMOTE_BUTTON_UP);
	//	printf("UP pressed\n");
	}
	if (IS_PRESSED(wm, WIIMOTE_BUTTON_DOWN)){
		make_input(WIIMOTE_BUTTON_DOWN);
	//	printf("DOWN pressed\n");
	}
	if (IS_PRESSED(wm, WIIMOTE_BUTTON_LEFT)){
		make_input(WIIMOTE_BUTTON_LEFT);
	//	printf("LEFT pressed\n");
	}
	if (IS_PRESSED(wm, WIIMOTE_BUTTON_RIGHT)){
		make_input(WIIMOTE_BUTTON_RIGHT);
	//	printf("RIGHT pressed\n");
	}
	if (IS_PRESSED(wm, WIIMOTE_BUTTON_MINUS)){
//		make_input(WIIMOTE_BUTTON_MINUS);
	//	printf("MINUS pressed\n");
	}
	if (IS_PRESSED(wm, WIIMOTE_BUTTON_PLUS)){
//		make_input(WIIMOTE_BUTTON_PLUS);
	//	printf("PLUS pressed\n");
	}
	if (IS_PRESSED(wm, WIIMOTE_BUTTON_ONE)){
		make_input(WIIMOTE_BUTTON_ONE);
	//	printf("ONE pressed\n");
	}
	if (IS_PRESSED(wm, WIIMOTE_BUTTON_TWO)){
		make_input(WIIMOTE_BUTTON_TWO);
	//	printf("TWO pressed\n");
	}
	if (IS_PRESSED(wm, WIIMOTE_BUTTON_HOME)){
		make_input(WIIMOTE_BUTTON_HOME);
	//	printf("HOME pressed\n");
	}
...
*/
	...
	//	printf("wm->accel.x = %f,wm->last_x = %f\n", (float)wm->accel.x, (float)wm->last_x);
		//printf("wiimote yaw   = %f\n", wm->orient.yaw);
		/*if(wm->orient.horizon == 1){
			wm->orient.horizon == 0;
			make_input(WIIMOTE_BUTTON_UP);
			wiiuse_motion_sensing(wm, 1);
		}*/
		if((int)wm->last_x != 0){
			if( (float)wm->accel.x - (float)wm->last_x > 50 && (float)wm->accel.x > (float)wm->last_x){
			//	printf("wiimote accel.x = %f last_x = %f\n", (float)wm->accel.x, (float)wm->last_x );
				wiiuse_motion_sensing(wm, 0);
				make_input(WIIMOTE_BUTTON_LEFT);
				wm->last_x = 0;
				flag = 0;
			}
			else if( (float)wm->last_x - (float)wm->accel.x > 50 && (float)wm->accel.x < (float)wm->last_x){
			//	printf("wiimote accel.x = %f last_x = %f\n", (float)wm->accel.x, (float)wm->last_x );
				wiiuse_motion_sensing(wm, 0);
				make_input(WIIMOTE_BUTTON_RIGHT);
				wm->last_x = 0;
...
*/
	...
	while (1) {
		...
		elapsedtime_buf[count] = end - start;
		forktime_buf[count] = forktime_a - forktime_b;
		count++;
	//	printf("elapsed time = %f\n", end - start);
	//	printf("forktime = %f\n",forktime_a - forktime_b);
		}
	}
